"""Tests for SubtileBasedLogicalScheduler — validates each scheduling pass.
...
class TestRemoveUnnecessaryWaitLrSync:
    ...
    def test_bf16_1x1(self):
        ...
        for slot in parts[0]:
            for gr in slot.grs:
                for op in gr.preOps:
                    assert op.kind in ('wait_lr_sync', 'sync', 'gr_inc', 'lr_inc',
                                        'wait_gr', 'wait_gr_sync', 'wait_lr', 'skip')
