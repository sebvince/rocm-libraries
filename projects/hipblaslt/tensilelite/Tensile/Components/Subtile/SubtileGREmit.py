...
def initTDMDescriptorSubtile(writer, kernel, tP):
  ...
  # so the TDM load broadcasts to peer workgroups' LDS. Subtile issues both A
  # and B on every wave, so it uses the split per-tensor masks (MulticastMaskA/
  # MulticastMaskB), not the non-subtile single parity mask. Masks are computed
  # once in defineAndResources.
