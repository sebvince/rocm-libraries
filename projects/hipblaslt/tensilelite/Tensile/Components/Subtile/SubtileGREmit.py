...
def initTDMDescriptorSubtile(writer, kernel, tP):
  ...
  with writer.allocTmpSgpr(1) as tmpSgprRes:
    ...
    # double-buffer it is the per-operand base stride ldsSwapDelta{tc} ([B0|A0|A1|B1]).
    swapMaskSgpr = f"tdmLdsSwapMask{tc}"
    swapDelta = (getattr(writer, "ldsSwapDelta%s" % tc) if getattr(writer, "overlapActive", False)
                 else writer.ldsTotalSize)
